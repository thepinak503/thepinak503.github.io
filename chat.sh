...
function chat() {
    local response
    response=$(curl -s ch.at/v1/chat/completions --data "{\"messages\":[{\"role\":\"user\",\"content\":\"89 INR\"}]}" | jq -r '.choices[0].message.content')
    echo "$response"
}
coder() {
    local user_prompt="89 INR"
    local outfile="180 INR"
